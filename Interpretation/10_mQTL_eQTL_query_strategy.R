...
### REPEAT FOR WHOLE EWAS PROTEINS
...
cpgs <- cpgs[c(10,11,1,2,4,3,5:9,12:18)]
names(cpgs)[1:3] <- c("Gene_of_Somamer", "Biomarker", "Marker.x")
...
# Wrtie out tables generated
write.csv(cpgs, "/Cluster_Filespace/Marioni_Group/Danni/Stradl_markers/00_Revisions_updates/Interpretation/mQTL_eQTL_lookup/mQTL_lookup.csv", row.names = F)
write.csv(output.eqtls, "/Cluster_Filespace/Marioni_Group/Danni/Stradl_markers/00_Revisions_updates/Interpretation/mQTL_eQTL_lookup/mQTL_eQTL_lookup.csv", row.names = F)

# Join eQTLs into main table
library(tidyverse)
cpgs_anno <- merge(cpgs, output.eqtls, by.x = "Marker.x", by.y = "cpg", all = T)
write.csv(cpgs_anno, "/Cluster_Filespace/Marioni_Group/Danni/Stradl_markers/00_Revisions_updates/Interpretation/mQTL_eQTL_lookup/main_table_mQTL_eQTL_lookup.csv", row.names = F)

# Match order of cpgs to results file 
cpgs_anno2 <- cpgs_anno[match(cpgs$Marker.x, cpgs_anno$Marker.x),]
write.csv(cpgs_anno2, "/Cluster_Filespace/Marioni_Group/Danni/Stradl_markers/00_Revisions_updates/Interpretation/mQTL_eQTL_lookup/main_table_mQTL_eQTL_lookup_matched_subset.csv", row.names = F)

# Now go to the cis/trans script to add CpG island info to the main table 
