...
for (j in 1:length(phenotype)){
  ...
  write.csv(results, paste0("/Cluster_Filespace/Marioni_Group/Danni/Stradl_markers/00_Revisions_updates/PheWAS_run2/Covariates/", pheno, "_result.csv"))
}
...
for (j in 1:length(set)){
  ...
  for(i in 1:4235){
    tryCatch({ 
      ...
      mod <- lmekin(scale(prot[,prot_name]) ~ scale(prot$st_age) + as.factor(prot$sex) + as.factor(prot[,pheno]) + as.factor(prot$combined) + as.factor(study_site) + as.factor(lag_group) + (1|prot$GS_id), data = prot, varlist = kin_model*2)
